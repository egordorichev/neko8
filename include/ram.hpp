 ...
 * 0x0041 - transparent colors (2 bytes)
 * 0x0043 - camera position (4 bytes)
 * 0x0047 - camera position inverted (1 byte)
 */
...
#define RAM_SIZE (SPRITE_END + MAP_SIZE \
	+ FLAGS_SIZE + SFX_SIZE + MUSIC_SIZE + PERSISTENT_SIZE + CODE_SIZE + DRAW_SIZE \
	+ VRAM_SIZE + OTHER_SIZE)

// Cart size
#define CART_SIZE (SPRITE_END + MAP_SIZE \
	+ FLAGS_SIZE + SFX_SIZE + MUSIC_SIZE + PERSISTENT_SIZE + CODE_SIZE)
