...
-- Updated for neko8 by egordorichev

local _ColorSet, _GIFScale, _LIKO_W, _LIKO_H = config.palette, 2,
  ...
  local dataget = data.getPixel
	...
	for y=y0, y1 do
		for x=x0, x1 do
			...
			local index=strchar(palmap[lshift(r, 16)+lshift(g, 8)+b]
			or error("R:"..tostring(r).." G:"..tostring(g).." B:"..tostring(b).." X:"..x.." Y:"..y)) --FIXME PLEASE
